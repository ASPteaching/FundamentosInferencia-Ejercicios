...
Para entender la solución de este ejercicio y otros similares debemos recordar que, como regla básica, si dos conjuntos no tienen ningún elemento común, podemos determinar el número de elementos de su unión (los que estan en uno **u** en otro) mediante la suma de los elements da uno. 

En cambio, si dichos conjuntos tienen algún elemento en común, es decir si su *intersección* no está vacía, sucedería que, si sumamos los elementos de ambos conjuntos, los elementos de dicha intersección se añadirían dos veces, por lo que para determinar el total de elementos de la unión debemos sumar los elementos de cada conjunto y restarle una vez los de la intersección.

Los diagramas de Venn nos ayudan a visualizar las diversas situaciones con que nos podemos encontrar.
...
```{r, echo=FALSE}
plot.new()
plot.window(xlim=c(-2,2), ylim=c(-1.5,1.5), asp=1)
rect(-2,-1.5,2,1.5, border="black", lwd=2)  # univers

# Círculs A i B
symbols(-0.7, 0, circles=1, inches=FALSE, add=TRUE, fg="black")
symbols( 0.7, 0, circles=1, inches=FALSE, add=TRUE, fg="black")

# Ombregem la part de B fora d'A (A^c ∩ B)
symbols(0.7, 0, circles=1, inches=FALSE, add=TRUE,
        bg=rgb(0.2,0.5,1,0.3), fg=NA)

# Ombregem la intersecció A ∩ B en un altre color
symbols(-0.7, 0, circles=1, inches=FALSE, add=TRUE,
        bg=rgb(1,0.2,0.2,0.3), fg=NA)

# Tornem a dibuixar els contorns
symbols(-0.7, 0, circles=1, inches=FALSE, add=TRUE, fg="black")
symbols( 0.7, 0, circles=1, inches=FALSE, add=TRUE, fg="black")

# Etiquetes dels conjunts
text(-0.7,1.2,"A", cex=1.2)
text( 0.7,1.2,"B", cex=1.2)

# Títol en notació matemàtica
